...
// add "inline" to the head of all functions (2011/01/17)
...
inline void reverseDcm( double *dcm1, double *dcm2 )
{
  dcm2[0] = -dcm1[0];
  dcm2[1] = -dcm1[1];
  dcm2[2] = -dcm1[2];
...
inline void reverseYpr( double *dcm1, double *dcm2 )
{
  dcm2[0] = dcm1[0];
  dcm2[1] = -dcm1[1];
  dcm2[2] = dcm1[2];
...
inline bool compareDcm( double* dcm1, double* dcm2 )
{
  for( int i(0); i!=9; ++i )
    {
      if( dcm1[i] != dcm2[i] )
...
inline void ypr2yprR(double *inYpr, double *reversedYpr)
{
...
inline void rotY(double *dcm,double theta)
{
  dcm[0]=cos(theta);
  dcm[1]=0;
  dcm[2]=-sin(theta);
...
inline void rotate( double *dcm, double *x, double *y, double *z )
{
  double x0(*x);
  double y0(*y);
  double z0(*z);
...
inline void getOneDcm(double *Dcm)
{
  Dcm[0]=Dcm[4]=Dcm[8]=1;
  Dcm[1]=Dcm[2]=Dcm[3]=Dcm[5]=Dcm[6]=Dcm[7]=0;
}

inline void ypr2dcm(double *ypr, double *dcm)
{
  double sy(sin(ypr[0]));
  double sp(sin(ypr[1]));
  double sr(sin(ypr[2]));
...
inline void dcm2ypr(double *dcm, double *ypr)
{
...
inline void dcmXdcm(double *dcm1, double *dcm2, double *dcm3)
{
  double work[9];
...
inline void dcmXdcmt(double *Dcm1,double *Dcmt,double *dcm3)
{
...
inline void rotXrotY(double *dcm, double x, double y)
{
  double tmp[9];
  double cx(cos(x));
  double sx(sin(x)); 
...
inline double pinPoint(double *pt1, double *pt2, char surface, double radius,
