...
#include <limits.h> // add for UINT_MAX (2010/12/27)

// #include <Model.hpp>// comment out (2010/12/27)
...
// comment out for test (2011/02/14)
...
void SpatiocyteStepper::initPriorityQueue()
{
  ...
  for(vector<Process*>::const_iterator i(theProcessVector.begin());
    ...
    {      
      ...
      std::string aClassName(aProcess->getProcessName());// temporary change code (2011/01/26)
      // std::string aClassName(aProcess->getPropertyInterface().getClassName());
...
inline void SpatiocyteStepper::step()
{
  ...
  setCurrentTime(thePriorityQueue.getTop()->getTime());
...
{ 
  ...
  for( System::Systems::const_iterator i( aSystem->getSystems().begin() ) ;
        i != aSystem->getSystems().end() ; ++i )
...
void SpatiocyteStepper::setCompartmentsProperties()
{
  ...
  for(unsigned int i(0); i != theCompartments.size(); ++i)
    {
      // change code (2011/02/01)
      cout << theCompartments[i]->system->getSystemName() << ":"
           << theCompartments[i]->system->getID() << endl;
...
void SpatiocyteStepper::registerCompartmentSpecies(Compartment* aCompartment)
{
  ...
  // change code (2011/01/11)
  for( System::Variables::const_iterator i( aSystem->getVariables().begin() ) ;
        i != aSystem->getVariables().end() ; ++i )
...
void SpatiocyteStepper::setLatticeProperties()
{
  ...
  theHCPk = theNormalizedVoxelRadius/sqrt(3.0); 
  theHCPh = theNormalizedVoxelRadius*sqrt(8.0/3.0);
  theHCPl = theNormalizedVoxelRadius*sqrt(3.0);
      ...
      if(aCompartment->isSurface)
        {
          ...
          // change integer argument to double in sqrt (2011/01/31)
          cout << "  [" << int(aSpecArea*(6.0*sqrt(2.0)+4.0*sqrt(3.0)+3.0*sqrt(6.0))/
                              (72.0*VoxelRadius*VoxelRadius)) << 
      ...
      else
        {
          ...
          // change integer argument to double in sqrt (2011/01/31)
          cout << "  [" << int(aSpecVolume/(4.0*sqrt(2.0)*pow(VoxelRadius, 3))) << 
...
{
  // change code (2011/01/11)
  for( System::Variables::const_iterator i( aCompartment->system->getVariables().begin() ) ;
        i != aCompartment->system->getVariables().end() ; ++i )
  // FOR_ALL(System::Variables, aCompartment->system->getVariables())
    {
      Variable* aVariable(i->second);
      if(aVariable->getID() == "REACTIVE")
        {
          std::string aStringID(aVariable->getName()); 
